...
public class hands_angle_animation extends JPanel{
	double born_x[][] = new double[100][25]; //x座標を格納するための配列
	double born_y[][] = new double[100][25]; //y座標を格納するための配列
	//ディレクトリ指定
	File file =new File("video_json/marin_flip_2_result");
	File files[] = file.listFiles();
	ObjectMapper mapper = new ObjectMapper();
	int n=0;
	Timer timer;
	public hands_angle_animation() {
		setOpaque(false);
...
//			    g.dispose();
	...
	public static void convertFormat2Vector(String[]self,String[]msg) {
	...
	public static void main(String[] args) {
		...
		JFrame a = new JFrame();
		a.add(new hands_angle_animation());
		a.setSize(600,400);
		a.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		a.setVisible(true);
		
		double result1 = Math.cos(Math.toRadians(60));
	    System.out.println(result1);
	    System.out.println(Math.toDegrees(Math.acos(result1)));
